...
/// </para>
/// </summary>
public class SchemeProperty : Property
{
    /// <summary>
    ...
    public SchemeProperty(Design design)
        : base(
            design,
            design.View.GetType().GetProperty(nameof(View.Scheme)) ?? throw new Exception("Scheme property has changed name!"))
    {
    }

    /// <summary>
    /// Adds an assignment block of CodeDOM code to <paramref name="args"/>.  In the case
    /// of <see cref="SchemeProperty"/> this is a reference assignment to a privately declared
    /// <see cref="Scheme"/> within <see cref="CodeDomArgs.Class"/> e.g.:
    /// <code>this.btn1.Scheme = this.greenOnBlack;</code>
    /// </summary>
    /// <param name="args">Current state of the .Designer.cs file.</param>
    /// <remarks>If no explicit <see cref="Scheme"/> is set
    ...
    /// case of <see cref="SchemeProperty"/> this is a reference assignment to a privately declared
    /// <see cref="Scheme"/> within <see cref="CodeDomArgs.Class"/> e.g.:
    ...
    {
        var s = this.GetValue() as Scheme;

        var name = SchemeManager.Instance.GetNameForScheme(s
            ?? throw new Exception("GetRhs is only valid when there is a known Scheme"));
    ...
    /// <summary>
    /// Returns the explicitly declared <see cref="Scheme"/> on the <see cref="View"/>
    /// this <see cref="Property"/> is pointed at or null if the current value is inherited
    /// from a parent container.
    /// </summary>
    /// <returns>Explicitly declared <see cref="Scheme"/> or null.</returns>
    public override object? GetValue()
    {
        return this.Design.State.OriginalScheme ?? this.Design.View.GetExplicitScheme();
    }
    ...
    public override void SetValue(object? value)
    {
        ...
        this.Design.State.OriginalScheme = value as Scheme;
    }

    /// <summary>
    /// <para>Returns the human readable value for <see cref="Scheme"/> (if explicitly defined on
    /// the <see cref="View"/>) as tracked by <see cref="SchemeManager"/>.  This will be the
    /// private field name in the .Designer.cs e.g. "greenOnBlack".
    /// </para>
    /// <para>If no explicit <see cref="Scheme"/> is defined (i.e. value is inherited from parent
    /// container) then the string "(Inherited)" is returned.</para>
    /// </summary>
    /// <returns>Name of explicit <see cref="Scheme"/> as tracked by <see cref="SchemeManager"/>
    ...
    protected override string GetHumanReadableValue()
    {
        ...
        return SchemeManager.Instance.GetNameForScheme(s) ?? "Unknown Scheme";
    }
}
